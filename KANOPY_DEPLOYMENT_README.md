...
In order to push your Docker image to ECR and deploy to Kubernetes, you will need to configure the following four Drone secrets:

| Secret Name                | Purpose                             |
| -------------------------- | ----------------------------------- |
| `staging_kubernetes_token` | Deploy to staging Kanopy cluster    |
| `prod_kubernetes_token`    | Deploy to production Kanopy cluster |
| `ecr_access_key`           | Push Docker images to AWS ECR       |
| `ecr_secret_key`           | Push Docker images to AWS ECR       |

#### Option A: Automated Setup via CLI (Recommended)

Run the included setup script from your repo root. It installs the Drone CLI (if missing), prompts for your token, auto-detects the repo, and sets all 4 secrets in one shot:

```bash
./setup-drone-secrets.sh
```

If `DRONE_TOKEN` is not already exported, the script will prompt you to grab it from <https://drone.corp.mongodb.com/account>.

You can also pre-set the token to skip the prompt:

```bash
DRONE_TOKEN=your-token ./setup-drone-secrets.sh
```

The script will:

1. Install the Drone CLI via Homebrew (if not already installed)
2. Detect your repo from `git remote origin` (format: `mongodb-industry-solutions/<repo-name>`)
3. Switch kubectl contexts to staging and production to extract tokens
4. Run `drone secret update` (or `add`) for all 4 secrets

#### Option B: Manual Setup via Drone Web UI

You can also add secrets through the Drone web UI:

1. Go to `https://drone.corp.mongodb.com/mongodb-industry-solutions/<your-repo-name>`
2. Navigate to **Settings → Secrets**
3. Add each secret using the values from the commands below
...
Copy each command's output and paste it as the secret value in the Drone UI.
